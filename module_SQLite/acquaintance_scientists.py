...
# con = sq.connect('SQLite/scientists.db')  # Создание объекта соединения, для обеспечения доступа к БД и управления транзакциями
...
# ========= Вариант с использованием контекстного менеджера, автоматически закрывающего соединение
# with sq.connect('SQLite/scientists.db') as con:
#     cur = con.cursor()
...
# Удаление существующей таблицы (таблицы предыдущей сессии)
with sq.connect('SQLite/scientists.db') as con:
    cur = con.cursor()
...
with sq.connect('SQLite/scientists.db') as con:
    ...
    # Добавление множества записей, заполняя данные по определенным столбацам, перечисляя значения через запятую
    cur.execute('''INSERT INTO scientists (name, sex, date_of_born, date_of_death)
...
# ========= Применение различных методов для выполнения SQL-команд (сравнение методов execute и executemany) =========
with sq.connect('SQLite/scientists.db') as con:
    cur = con.cursor()

    # execute - единоразовый SQL-запрос с одним набором параметров. Применяется для разовой операции, например, создание таблицы, добавление или редактирование записей, но с применением только одного набора параметров
    ...
    # execute может включать одно или множество значений, переданных в одном наборе параметров коллекции кортежа или словаря. Значения набора параметров станут последовательно размещаться вместо соответствующих символов вопросительного знака в SQL-запросе
    cur.execute('''INSERT INTO scientists (name, sex, date_of_born, date_of_death)
    ...
    cur.execute('''INSERT INTO scientists (name, sex, date_of_born, date_of_death)
                VALUES ('Игорь Васильевич Курчатов', 1, ?, ?), ('Ландау Лев Давидович', 1, ?, ?)
                ''', (1903, 1960, 1908, 1968))


    # Для задействия множества наборов параметров, применяется метод executemany
    # executemany - выполняет SQL-запрос для каждого переданного набора параметров
    new_scientists = [('Ландау Лев Давидович', 1, 1908, 1968), ('Александр Михайлович Прохоров', 1, 1916, 2002)]
...
with sq.connect('SQLite/scientists.db') as con:
    ...
    # Изменение значения в столбце name на указанную строку для определенной записи с ROWID = 1 (первая запись в таблице)
