...
if __name__ == '__main__':
    ...
    print('Класс применяет лаконичное и короткое определение интерфейсов для внедрения логики для всех атрибутов целевого класса:')
    print('- Во время инициализации объекта pt3, осуществляется создание инстансов дескриптора Integer\n'
        'Т.е. каждый атрибут объекта станет являться инстансом дескриптора Integer (дескриптором атрибута)\n')
    print('- В инициализаторе целевого класса NewPoint3D, для каждого определяемого атрибута,\n' 
        'будет совершено обращение к одноименному дескриптору - объекту дескриптора.\n'
        'И присваивается, переданное в NewPoint3D значение атрибута\n'
        'Что вызывает сеттер класса дескриптора Integer (метод __set__), который задействует, заложенную в него логику')
    print('- В последующем, если станет производиться обращение к определенному атрибуту объекта NewPoint3D,\n'
        'то это вызовет соответствующий геттер класса дескриптора Integer (метод __get__)')
