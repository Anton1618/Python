'''Класс Point для реализации арифметических операций над элементами объектов:

- move_to - сеттер, который устанавливает значения координат x и y;
...
- get_distance - геттер, который вычисляет и возвращает расстояние между точками по теореме Пифагора
    - Обязательно принимает экземпляр класса Point;
    - Если в метод передается НЕ экземпляр класса Point, то возбуждается исключение с текстом.
...
class Point:
    FIELDS = ('x', 'y', 'list_points')  # Доступные атрибуты объекта
    ...
    def get_distance(self, point2):
        ...
        if not isinstance(point2, self.__class__): # Или point2.__class__.__name__ != 'Point'
...
if __name__ == '__main__':
    print(' Инициализация и получение дистанции между точками через пользовательские методы '.center(120, '-'))
    sp1 = Point(1, 2, True)
    sp2 = Point(4, 6, True)
    print(sp1.get_distance(sp2))
    sp2.move_to(5, 7)
    print(sp1.get_distance(sp2))
