...
class IterableContainer(MutableContainer):
    ...
    def __iter__(self):
        '''Метод реализует процесс итерирования элементов объекта через заданную логику выполнения в методе __next__
        Обычно возвращает только ссылку на объект - self
        Также, может включать инициирование счетчика-индекса, который применяется для:
        - задания начального индекса, при итерировании массива.
        - обращения к элементам массива, например, для их сохранения или введения условной логики отбора
        - как значение в условной конструкции, для прекращения итерирования.
    Счетчик-индекс может быть определен в методе __iter__, либо любом другом пространстве объекта, но тогда, должен быть
    переопределен в условной конструкции метода __next__, при наступлении StopIteration'''
        self.index = 0
        return self
    def __next__(self):
        '''Для исключения "ошибки" выхода за границы контейнера, применяется условие по длине массива, которое при
        возникновении, станет указывать функции iter() на прекращение итерирования
        '''
        if self.index >= len(self.values):
            # self.index = 0  # Переопределение счетчика, если он был реализована вне метода __iter__
            raise StopIteration
