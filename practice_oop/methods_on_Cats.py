...
if __name__ == '__main__':
    print(' Объект Tom инициализируется простой передачей параметров методу __init__ '.center(120, '-'))
    tom = BlueCat('Tom', 2)
    print(tom.meow())
    print()

    print(' Объект Angela инициализируется путем передачи параметров методу класса make_cat '.center(120, '-'))
    angela = BlueCat.make_cat('Angela')
    print(angela.meow())
    print()

    print(' Метод преобразования возраста кошки в человеческий. '.center(120, '-'))
    print('Метод либо вычисляет и возвращает значение, либо возвращает результат предыдущих вычислений')
