...
Так, например, в цикле для получения общего интерфейса взаимодействия, для вызова одного метода для каждого из объектов,
в которых реализуется свой функционал, не зависящий от других, что позволяет сократить код и исключить необходимость
обработки каждого типа данных по отдельности. А также применять полиморфную переменную самого объекта,
для задействия его магического метода
'''
