...
- Декорируемое свойство password имеет:
    - геттер - представляет собой вечный валидатор на соответствие пароля и пользовательского ввода, для получения
        доступа к нему.
        Важным моментом является то, что изначально поле _password задается объекту при инициализации, а в дальнейшем
        контролируется его управляющим свойством property.
        Property декорирует атрибут, заменяя таким образом ссылку на изначальный атрибут _password, ссылкой на новый
        публичный и разрешенный для применения
        Это управляемое свойство применяется для получения самого пароля, а также применяется в других методах как
        проверка, выполняя роль аутентификации пользователя
