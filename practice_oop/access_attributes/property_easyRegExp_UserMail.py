...
Описание:
- get_email - геттер, возвращающий защищенный атрибут _email,
во время выполнения команды obj.attr
- set_email - сеттер, устанавливающий переданную строку как новое значение почты,
во время выполнения команды obj.attr = value
...
# Требуется проверка почтового адреса перед установкой.
# В строке емейла должен быть только 1 символ @ и после ней есть точка.
# В противном случае возбудить исключение 'Почтовый адрес указан неверно'
import re
