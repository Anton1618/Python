'''Реализация паттерна monostate. Для сравнения, также введена закомментированная реализация паттерна singleton.

Monostate применяется для реализации одного состояния между всеми реализациями объектов класса.
Может применяться в дополнение к singleton, реализуя таким образом уникальность объекта в системе, не запрещая создание
новых объектов, но и не реализуя этого, а возвращая ранее созданный объект с установленным для него состоянием
'''
