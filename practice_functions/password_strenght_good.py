...
def password_strength(value: str) -> str:
    ...
    for symbols in (digits, ascii_lowercase, ascii_uppercase, punctuation):
        if any(i in symbols for i in value):
            ...
            # Для корректного расчета сложности пароля, необходим переход к новому набору из представленных, иначе сложность пароля превратиться в счетчик полученных значений
