...
- Содержит только один из наборов символов.
Если пароль содержит только цифры или только строчные, или только заглавные символы - вернуть Weak
- Содержит два любых набора символов.
Если пароль содержит символы любых 2 наборов - вернуть Good
- Содержит наборы всех символов.
Если пароль содержит символы всех наборов - вернуть Very Good
'''
...
def password_strength(value: str) -> str:
    ...
    for symbols in (digits, ascii_lowercase, ascii_uppercase, punctuation):
        if any(i in symbols for i in value):
            count += 1  # Встречая символ, который входит в текущий итерируемый набор, увеличивается сложность пароля.
            # Но это должно происходить только один раз за набор
            # Для корректного расчета сложности пароля, необходим переход к новому набору из представленных, иначе
            # сложность пароля превратиться в счетчик полученных значений
            continue  # Поэтому после получения уникального значения, производится переход к следующему набору символов
    if count == 4:  # Значение максимальной сложности пароля, которое может быть получено только при вхождении символов
        # пароля во все наборы символов
        return 'Very Good'
